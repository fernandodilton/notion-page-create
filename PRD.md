...
- **Social posts**: repositório de posts de redes sociais (Instagram, Twitter/X, TikTok e similares); `Name` e `Link` recebem a própria URL do post; verificação de duplicidade por `Link`; campo `Type` (multi_select) opcional com opções "Idea" e "Favorite", enviado via `type_selected` no payload como string separada por vírgula; verificação de scraper: offline → ARCHIVED; online → `executeHybridExtraction` para obter OG image como capa; URLs YouTube → YouTube Data API para thumbnail
